...
export const menteeLoginAPI = async (email, password) => {
...
//API that checks the user Credentials with slot id

export const checkSlotCredentials = async (slotId) => {
  try {
    const { menteeId } = getUserIdAndToken("menteeAuth");

    const responseFromAPI = await menteesAxiosInstance.post(
      BASE_URL + END_POINTS.MENTEES_VERIFY_LIVE,
      {
        userId: menteeId,
        slotId: slotId,
      }
    );
    console.log("Respose from live Verificatrion", responseFromAPI);
    return responseFromAPI?.status == "200" ? true : false;
  } catch (error) {
    console.log(error);
    throw error;
  }
