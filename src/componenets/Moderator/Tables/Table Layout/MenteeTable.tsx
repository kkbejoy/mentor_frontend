...
import { menteeBlockUnblockAPI } from "../../../../api/moderatorConfiguration/moderatorServices";
...
//Moderator -Mentees List- Make this into reusable components
const Table = ({ columns, data }) => {
  const disptch = useDispatch();

  const [isUpdated, setIsUpdated] = useState(false);
  //Function to block Mentees
  const handleButtonClick = (id) => {
    try {
      console.log("Block/unblock Button Clicked");
      const response = menteeBlockUnblockAPI(id);
      response.then((res) => {
    ...
    hooks.visibleColumns.push((columns) => [
      ...columns,
      {
        ...
        Cell: ({ row }) => (
          ...
          console.log(row.original.isBlocked ? "Blocked" : "Active"),
          (
